...
```{r}
MFdict <- go_dict[go_dict$ontology == "MF", ]
MFdict<- MFdict[,c(2,6)]

enrchdUP_GO<-enricher(gene, pAdjustMethod = "none", universe, pvalueCutoff = 0.05,TERM2GENE= MFdict)

upMF_GOs <- data.frame(enrchdUP_GO)
...
```{r}
CCdict <- go_dict[go_dict$ontology == "CC", ]
CCdict<- CCdict[,c(2,6)]

enrchdUP_GO<-enricher(gene, pAdjustMethod = "none", universe, pvalueCutoff = 0.05,TERM2GENE= CCdict)

upCC_GOs <- data.frame(enrchdUP_GO)

upCC_GOs
```

for David: https://david.ncifcrf.gov/gene2gene.jsp
```{r}
...
some usefull tools: 
- `kegga` function in edgeR package (bioconductor)
